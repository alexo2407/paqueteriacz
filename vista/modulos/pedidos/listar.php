...
<?php if ($isClienteParaBI && !$isAdmin && !empty($proveedoresMensajeriaCliente)): ?>
<style>
.prov-bi-wrap .proveedor-bi-card {
    border:none; border-radius:14px; padding:1.25rem 1.5rem;
    box-shadow:0 4px 16px rgba(0,0,0,.07); background:white;
    transition:transform .25s ease, box-shadow .25s ease;
}
.prov-bi-wrap .proveedor-bi-card:hover { transform:translateY(-3px); box-shadow:0 8px 24px rgba(0,0,0,.12); }
.prov-bi-wrap .prov-name  { font-weight:700; font-size:1rem; color:#1a1a2e; }
.prov-bi-wrap .prov-badge { font-size:1.7rem; font-weight:800; }
.prov-bi-wrap .ef-bar { height:8px; border-radius:50px; background:#f0f0f0; overflow:hidden; }
.prov-bi-wrap .ef-bar-inner { height:100%; border-radius:50px; transition:width .8s ease; }
.prov-bi-wrap .ef-green  { background:linear-gradient(90deg,#11998e,#38ef7d); }
.prov-bi-wrap .ef-yellow { background:linear-gradient(90deg,#f7971e,#ffd200); }
.prov-bi-wrap .ef-red    { background:linear-gradient(90deg,#f5576c,#f093fb); }
.prov-bi-wrap .rec-badge { display:inline-block; padding:.3rem .85rem; border-radius:50px; font-size:.78rem; font-weight:600; }
.prov-bi-wrap .rec-best  { background:rgba(17,153,142,.12); color:#11998e; }
.prov-bi-wrap .rec-worst { background:rgba(245,87,108,.12); color:#f5576c; }
</style>
<div class="prov-bi-wrap mb-4">
    <div class="card" style="border:none;border-radius:16px;box-shadow:0 4px 20px rgba(0,0,0,.06);overflow:hidden;">
        <div class="card-body p-4">
            <div class="d-flex justify-content-between align-items-start mb-3">
                <div>
                    <h5 class="mb-1"><i class="bi bi-truck text-info"></i> Mis Proveedores de Mensajería</h5>
                    <small class="text-muted">Efectividad de entrega · <?= date('d/m/Y', strtotime($fechaDesdeBI)) ?> al <?= date('d/m/Y', strtotime($fechaHastaBI)) ?></small>
                </div>
            </div>

            <?php if ($mejorProveedorCliente): ?>
            <div class="alert border-0 mb-4" style="background:linear-gradient(135deg,rgba(17,153,142,0.08),rgba(56,239,125,0.08));border-left:4px solid #11998e !important;border-radius:12px;">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div>
                        <div class="fw-bold text-success">Mejor proveedor: <strong><?= htmlspecialchars($mejorProveedorCliente['proveedor_nombre']) ?></strong></div>
                        <div class="small text-muted"><?= $mejorProveedorCliente['efectividad'] ?>% de efectividad · <?= $mejorProveedorCliente['entregados'] ?> de <?= $mejorProveedorCliente['total_pedidos'] ?> pedidos entregados</div>
                    </div>
                </div>
            </div>
            <?php endif; ?>
            ...
            <div class="row g-3">
            <?php foreach ($proveedoresMensajeriaCliente as $idx => $prov):
            ...
            <div class="col-md-6 col-xl-4">
                <div class="proveedor-bi-card h-100">
                    ...
                    <div class="ef-bar mb-3">
                        <div class="ef-bar-inner <?= $barClass ?>" style="width:<?= min($ef,100) ?>%"></div>
                    </div>
                    ...
                    <?php endif; ?>
                    <?php if ($idx === 0 && count($proveedoresMensajeriaCliente) > 1): ?>
                    <div class="mt-2"><span class="rec-badge rec-best">✔ Mejor rendimiento</span></div>
                    <?php elseif ($idx === count($proveedoresMensajeriaCliente) - 1 && count($proveedoresMensajeriaCliente) > 1): ?>
                    <div class="mt-2"><span class="rec-badge rec-worst">⚠ Pendiente de mejora</span></div>
                    <?php endif; ?>
                </div>
            </div>
            <?php endforeach; ?>
