...
class ColectaServiceIntegrationTest extends TestCase
{
    ...
    public function test_abrir_colecta_con_pedidos_esperados(): void
    {
        ...
        $operador = \LogisticaTestDataFactory::crearUsuario($this->db, 'operador');
        $p1 = \LogisticaTestDataFactory::crearPedido($this->db, $cliente);
        $p2 = \LogisticaTestDataFactory::crearPedido($this->db, $cliente);

        $resultado = $this->servicio()->abrirColecta($cliente, '2099-01-15', 'MANANA', $operador);
    ...
    /** @test 2. Rechazar colecta duplicada para mismo cliente, fecha y turno. */
    public function test_rechaza_colecta_duplicada(): void
    {
        $cliente  = \LogisticaTestDataFactory::crearUsuario($this->db, 'cliente');
        $operador = \LogisticaTestDataFactory::crearUsuario($this->db, 'operador');
        $svc = $this->servicio();
        $svc->abrirColecta($cliente, '2099-01-16', 'TARDE', $operador);
    ...
    public function test_rechaza_turno_invalido(): void
    {
        ...
        $this->expectException(\LogisticaOperativaException::class);
        $this->servicio()->abrirColecta($cliente, '2099-01-17', 'NOCHE', $operador);
    }
    ...
    public function test_rechaza_fecha_invalida(): void
    {
        ...
        $this->expectException(\LogisticaOperativaException::class);
        $this->servicio()->abrirColecta($cliente, '15/01/2099', 'MANANA', $operador);
    }
    ...
    public function test_escaneo_pedido_esperado_resulta_recibido(): void
    {
        ...
        $svc     = $this->servicio();
        $colecta = $svc->abrirColecta($cliente, '2099-02-01', 'MANANA', $operador);
    ...
    public function test_escaneo_pedido_no_esperado_resulta_extra(): void
    {
        ...
        $pedidoExtra = \LogisticaTestDataFactory::crearPedido($this->db, $clienteB);

        $svc     = $this->servicio();
        $colecta = $svc->abrirColecta($cliente, '2099-02-02', 'TARDE', $operador);
    ...
    public function test_mismo_uuid_es_idempotente(): void
    {
        ...
        $svc     = $this->servicio();
        $colecta = $svc->abrirColecta($cliente, '2099-02-03', 'MANANA', $operador);
        $uuid    = \LogisticaTestDataFactory::uuid();
    ...
    public function test_mismo_evento_uuid_diferente_es_idempotente(): void
    {
        ...
        $svc     = $this->servicio();
        $colecta = $svc->abrirColecta($cliente, '2099-02-04', 'TARDE', $operador);
    ...
    public function test_cierre_identifica_faltantes(): void
    {
        $cliente  = \LogisticaTestDataFactory::crearUsuario($this->db, 'cliente');
        $operador = \LogisticaTestDataFactory::crearUsuario($this->db, 'operador');
        $p1 = \LogisticaTestDataFactory::crearPedido($this->db, $cliente); // será RECIBIDO
        $p2 = \LogisticaTestDataFactory::crearPedido($this->db, $cliente); // será FALTANTE

        $svc     = $this->servicio();
        $colecta = $svc->abrirColecta($cliente, '2099-03-01', 'MANANA', $operador);
    ...
    public function test_segundo_cierre_es_rechazado(): void
    {
        ...
        $svc     = $this->servicio();
        $colecta = $svc->abrirColecta($cliente, '2099-03-02', 'TARDE', $operador);
    ...
    public function test_contadores_coinciden_con_registros_reales(): void
    {
        ...
        $p1 = \LogisticaTestDataFactory::crearPedido($this->db, $cliente);
        $p2 = \LogisticaTestDataFactory::crearPedido($this->db, $cliente);
        $p3 = \LogisticaTestDataFactory::crearPedido($this->db, $cliente);

        $svc       = $this->servicio();
        $colecta   = $svc->abrirColecta($cliente, '2099-03-03', 'MANANA', $operador);
    ...
    public function test_pedido_id_estado_no_cambia(): void
    {
        ...
        $svc     = $this->servicio();
        $colecta = $svc->abrirColecta($cliente, '2099-04-01', 'MANANA', $operador);
    ...
    public function test_stock_no_recibe_movimientos(): void
    {
        ...
        $svc     = $this->servicio();
        $colecta = $svc->abrirColecta($cliente, '2099-04-02', 'TARDE', $operador);
        $this->escanear($svc, $colecta['id_colecta'], $pedido, $operador);
        $svc->cerrarYConciliar($colecta['id_colecta'], $operador);
    ...
    public function test_inventario_no_cambia(): void
    {
        ...
        $svc     = $this->servicio();
        $colecta = $svc->abrirColecta($cliente, '2099-04-03', 'MANANA', $operador);
        $this->escanear($svc, $colecta['id_colecta'], $pedido, $operador);
        $svc->cerrarYConciliar($colecta['id_colecta'], $operador);
    ...
    public function test_reservas_no_cambian(): void
    {
        ...
        $svc     = $this->servicio();
        $colecta = $svc->abrirColecta($cliente, '2099-04-04', 'TARDE', $operador);
        $this->escanear($svc, $colecta['id_colecta'], $pedido, $operador);
    ...
    public function test_datos_visibles_dentro_de_transaccion(): void
    {
        ...
        $cliente  = \LogisticaTestDataFactory::crearUsuario($this->db, 'cliente');
        $operador = \LogisticaTestDataFactory::crearUsuario($this->db, 'operador');
        $this->servicio()->abrirColecta($cliente, '2099-05-01', 'MANANA', $operador);
