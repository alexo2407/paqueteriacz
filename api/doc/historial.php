...
if (isset($_GET['ver'])) {
    ...
    if ($doc) {
        ...
        echo '.print-url-link{color:#0B4EA2;font-weight:600}';
        echo '.print-section{margin-bottom:28px}';
        echo '.print-section-title{font-size:15px;font-weight:700;margin-bottom:10px;color:#061C4C;border-left:3px solid #FF8A00;padding-left:8px}';
        echo '.print-endpoint-box{background:#061C4C;border-radius:8px;padding:12px 16px;margin-bottom:10px;display:flex;align-items:center;justify-content:space-between;color:#fff}';
        ...
        echo '.print-method.post{background:#10b981}.print-method.get{background:#0B4EA2}';
        ...
        echo '.print-badge-auth.public{background:#10b981}.print-badge-auth.auth{background:#0B4EA2}';
        echo '.print-table{width:100%;border-collapse:collapse;margin-bottom:12px;font-size:12px}';
        echo '.print-table th{background:#EEF2F6;color:#061C4C;padding:7px 10px;text-align:left;font-weight:600;text-transform:uppercase;font-size:11px}';
        echo '.print-table td{padding:6px 10px;border-bottom:1px solid #e5e7eb}';
        echo '.print-table code{background:#f3f4f6;color:#0B4EA2;padding:1px 5px;border-radius:3px;font-family:Fira Code,monospace}';
        echo '.print-code-block{background:#061C4C;border-radius:6px;padding:12px 14px;font-family:Fira Code,monospace;font-size:11px;color:#e2e8f0;white-space:pre-wrap;word-break:break-all;margin-bottom:10px}';
        ...
        echo '.print-tip{background:#EEF2F6;border-left:3px solid #0B4EA2;padding:8px 12px;font-size:11px;color:#061C4C;border-radius:0 4px 4px 0;margin-bottom:10px}';
        ...
        /* Barra superior de la vista de impresión con branding RutaEx */
        echo '.no-print-bar{background:linear-gradient(135deg,#061C4C 0%,#0B4EA2 100%);color:#fff;padding:12px 24px;display:flex;gap:12px;align-items:center;margin:-30px -40px 30px;font-size:13px}';
        echo '.no-print-bar .brand{display:flex;align-items:center;gap:10px;font-weight:700;font-size:16px;letter-spacing:.5px}';
        echo '.no-print-bar .brand-arrow{color:#FF8A00;font-size:18px}';
        echo '.no-print-bar .brand-dot{color:#FF8A00}';
        echo '.no-print-bar .doc-name{opacity:.75;font-size:12px;font-weight:400;border-left:1px solid rgba(255,255,255,.3);padding-left:12px;margin-left:4px}';
        echo '.no-print-bar .spacer{flex:1}';
        echo '.no-print-bar a,.no-print-bar button{color:#fff;text-decoration:none;font-weight:600;background:rgba(255,255,255,0.12);border:1px solid rgba(255,255,255,0.25);cursor:pointer;font-size:12px;padding:6px 16px;border-radius:6px;transition:background .2s}';
        echo '.no-print-bar a:hover,.no-print-bar button:hover{background:rgba(255,255,255,0.25)}';
        echo '@media print{.no-print-bar,.no-print{display:none!important}}';
        echo '</style></head><body>';
        echo '<div class="no-print-bar">';
        echo '<div class="brand"><span class="brand-arrow">&#9658;</span> RutaEx<span class="brand-dot">.</span>Latam</div>';
        echo '<span class="doc-name">' . htmlspecialchars($doc['titulo']) . '</span>';
        echo '<div class="spacer"></div>';
        echo '<button onclick="window.print()">&#128438; Exportar PDF</button>';
